...
def delete_devices() -> None:
    ...
    if devices_table.selected:
        db_full = list(filter(lambda row: row not in devices_table.selected, db_full))
        app.storage.general["db"] = db_full
        devices_table.remove_rows(devices_table.selected)
        apply_filters()
        update_ui()
    else:
        ui.notify(message="Выберите устройства", position="top", type="warning")
...
def show_discover_dialog() -> ui.dialog:
    ...
    if btn_discover.icon == "search":
        with ui.dialog(value=True) as discover_dialog, ui.card():
            ...
            with ui.row():
                ui.button(
                    ...
                    on_click=lambda: get_subnet(
                        discover_dialog, subnet.value, clear_db_switch.value
                    ),
                )
                ui.button("Отмена", on_click=discover_dialog.close)
            if dark_mode.value:
        ...
        apply_filters()
        ui.notify(
            message="Обнаружение устройств прекращено", position="top", type="info"
        )
...
async def get_subnet(dialog: ui.dialog, subnet: str, clear: bool) -> None:
    ...
    if is_ip_subnet(subnet):
        ...
        discover_tasks = [
            asyncio.create_task(discover_device(host, semaphore)) for host in hosts
        ]
        ...
        apply_filters()
        ui.notify(
            message="Обнаружение устройств завершено", position="top", type="positive"
        )
    else:
        ui.notify(message="Введите адрес подсети", position="top", type="warning")
...
def show_configure_dialog() -> ui.dialog:
    ...
    if btn_configure.icon == "tune":
        with ui.dialog(value=True) as configure_dialog:
            with ui.card():
                ...
                with ui.row():
                    ...
                    ui.button("Отмена", on_click=configure_dialog.close)
    else:
        ...
        devices_table.selected = []
        lbl_total_selected.set_text("0")
        ui.notify(message="Передача команд прекращена", position="top", type="info")
...
async def send_commands(dialog: ui.dialog, commands: str) -> None:
    ...
    if commands:
        ...
        ui.notify(
            message="Введите команды для выполнения", position="top", type="warning"
        )
    ...
    if devices_count > 0:
        if rows_per_page > 0:
            ...
            devices_table.update_rows(
                db_filtered[
                    (current_page - 1) * rows_per_page : current_page * rows_per_page
                ]
            )
        else:
...
if __name__ in {"__main__", "__mp_main__"}:
    ...
    rows_per_page = app.storage.general.get("rows_per_page", config.ROWS_PER_PAGE)
    ...
    if rows_per_page not in config.ROWS_COUNT_OPTIONS:
    ...
    with ui.header():
        ...
        ui.space()
        ui.separator()
        ui.button(icon="first_page", on_click=goto_first_page).tooltip(
            "Первая страница"
        )
        ui.button(icon="chevron_left", on_click=goto_previous_page).tooltip(
            "Предыдущая страница"
        )
        lbl_active_pages = ui.label("0 из 0")
        ui.button(icon="chevron_right", on_click=goto_next_page).tooltip(
            "Следующая страница"
        )
        ui.button(icon="last_page", on_click=goto_last_page).tooltip(
            "Последняя страница"
        )
        ui.separator()
        rows_per_page_dropdown = (
            ...
            .props(add='dark borderless options-dark="false"', remove="outlined")
        ...
        if 0 not in rows_per_page_dropdown.options:
            rows_per_page_dropdown.options.insert(0, 0)
        ui.separator()
        ui.button(icon="delete_outline", on_click=delete_devices).tooltip(
            "Удаление устройств"
        )
        ...
        btn_configure = ui.button(
            ...
            on_click=lambda: (
                ...
                if len(devices_table.selected) > 0
                    ...
                    message="Выберите устройства", position="top", type="warning"
                )
            ),
        ).tooltip("Конфигурирование")
        btn_discover = ui.button(icon="search", on_click=show_discover_dialog).tooltip(
            "Начать обнаружение"
        )
        ui.separator()
        btn_mode = ui.button(icon="dark_mode", on_click=change_ui_mode).tooltip(
            "Темный"
        )
    ...
    with ui.right_drawer(fixed=True, value=True) as filters_drawer:
        ...
        with ui.column() as filters_block:
            lst_categories = ui.select(
                [], on_change=apply_filters, label="Категория"
            ).props("clearable")
            lst_vendors = ui.select(
                [], on_change=apply_filters, label="Производитель"
            ).props("clearable")
            lst_models = ui.select([], on_change=apply_filters, label="Модель").props(
                "clearable"
            )
            telnet_switch = ui.switch(
                "Включен протокол telnet", value=True, on_change=apply_filters
            )
            ssh_switch = ui.switch(
                "Включен протокол ssh", value=True, on_change=apply_filters
            )
        ui.space()
    ...
    with ui.table(
        ...
        row_key="host",
        selection="multiple",
        on_select=lambda: lbl_total_selected.set_text(len(devices_table.selected)),
    ) as devices_table:
        devices_table.add_slot(
            "body-cell-snmp",
            """
